function [count, meanval, edges, id] = histcn(X, varargin)
...
count = zeros(bin*bin,1);
ct = accumarray(id, 1);
mc = size(ct,1);
count(1:mc) = ct;
mv = zeros(mc, size(AccumData,2));
for i = 1:size(AccumData,2)
    mv(:,i) = accumarray(id, AccumData(:, i), [], Fun{:});
end
meanval = zeros(bin*bin,2);
meanval(1:mc,:) = mv;
